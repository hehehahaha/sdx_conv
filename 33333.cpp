...
#pragma HLS array_partition variable=tempA complete dim =1
	haha tempB[OUT_CH_NUM_Conv / 32][IN_CH_NUM_Conv][Conv_MAP_SIZE][Conv_MAP_SIZE];
#pragma HLS array_partition variable=tempB complete dim =1
#pragma HLS array_partition variable=tempA complete dim =2
	haha tempconv1out[OUT_CH_NUM_Conv / 32][OUT_HEIGHT_Conv][OUT_WIDTH_Conv] = { 0 };
...
#pragma HLS array_partition variable=tempburelu1out complete dim =1
	haha tempW2[OUT_CH_NUM_Conv2 / 32][OUT_CH_NUM_Conv][Conv_MAP_SIZE][Conv_MAP_SIZE];
#pragma HLS array_partition variable=tempW2 complete dim =1
#pragma HLS array_partition variable=tempW2 complete dim =2
	haha tempconv2out[OUT_CH_NUM_Conv2 / 32][OUT_HEIGHT_Conv2][OUT_WIDTH_Conv2] = { 0 };
#pragma HLS array_partition variable=tempconv2out complete dim =1
	...
	for (int cyc = 0; cyc < OUT_CH_NUM_Conv2; cyc += OUT_CH_NUM_Conv2 / 32)
	{
		...
		memcpy((void*)tempW2, W2 + Conv_MAP_SIZE * Conv_MAP_SIZE * OUT_CH_NUM_Conv * cyc , sizeof(haha) * OUT_CH_NUM_Conv2 * OUT_CH_NUM_Conv * Conv_MAP_SIZE * Conv_MAP_SIZE / 32);
		...
		for (int kr = 0; kr < Conv_MAP_SIZE; kr++)//3
		{
			for (int kc = 0; kc < Conv_MAP_SIZE; kc++)//3
			{
				for (int r = 0; r < OUT_HEIGHT_Conv2; r++)//28
				{
					...
					{
#pragma HLS PIPELINE
						for (int cho = 0; cho < OUT_CH_NUM_Conv2 / 32; cho++)//64
						{
		...
		memcpy(conv2out + cyc * OUT_HEIGHT_Conv2 * OUT_WIDTH_Conv2, (const void*)tempconv2out, sizeof(haha) * OUT_CH_NUM_Conv2 / 32 * OUT_HEIGHT_Conv2 * OUT_WIDTH_Conv2);
...
#pragma SDS data zero_copy(conv3out[0:OUT_CH_NUM_Conv3 / 64 * OUT_HEIGHT_Conv3 * OUT_WIDTH_Conv3], gama3[0:OUT_CH_NUM_Conv3 / 64], beta3[0:OUT_CH_NUM_Conv3 / 64], burelu3out[0:OUT_CH_NUM_Conv3 / 64 * OUT_HEIGHT_Conv3 * OUT_WIDTH_Conv3])
void bnrelu3(haha *conv3out, haha *gama3, haha *beta3, haha *burelu3out) {
	haha tempconv3out[OUT_CH_NUM_Conv3 / 64][OUT_HEIGHT_Conv3][OUT_WIDTH_Conv3];
	haha tempgama3[OUT_CH_NUM_Conv3 / 64];
	haha tempbeta3[OUT_CH_NUM_Conv3 / 64];
	haha tempbn3[OUT_CH_NUM_Conv3 / 64][OUT_HEIGHT_Conv3][OUT_WIDTH_Conv3] = { 0 };
	//haha tempburelu1out[OUT_CH_NUM_Conv][OUT_HEIGHT_Conv][OUT_WIDTH_Conv] = { 0 };
	for (int cyc = 0; cyc < OUT_CH_NUM_Conv3; cyc += OUT_CH_NUM_Conv3 / 64)
	{
		...
		memcpy(burelu3out + cyc * OUT_HEIGHT_Conv3 * OUT_WIDTH_Conv3, (const void*)tempbn3, sizeof(haha) * OUT_CH_NUM_Conv3 * OUT_HEIGHT_Conv3 * OUT_WIDTH_Conv3 / 64);
...
#pragma HLS array_partition variable=temppooling2out complete dim =1
	haha tempW5[OUT_CH_NUM_Conv5 / 512][OUT_CH_NUM_Conv4][Conv_MAP_SIZE][Conv_MAP_SIZE];
#pragma HLS array_partition variable=tempW5 complete dim =1
#pragma HLS array_partition variable=tempW5 complete dim =2
	haha tempconv5out[OUT_CH_NUM_Conv5 / 512][OUT_HEIGHT_Conv5][OUT_WIDTH_Conv5] = { 0 };
...
#pragma HLS PIPELINE
						for (int cho = 0; cho < OUT_CH_NUM_Conv5 / times; cho++)//512/64
						{
							...
							{
								tempconv5out[cho][r][c] += temppooling2out[chi][r + kr][c + kc] * tempW5[cho][chi][kr][kc];
							}
						}
					}
				}
...
#pragma HLS array_partition variable=tempburelu5out complete dim =1
	haha tempW6[OUT_CH_NUM_Conv6 / 128][OUT_CH_NUM_Conv5][Conv_MAP_SIZE][Conv_MAP_SIZE];
#pragma HLS array_partition variable=tempW6 complete dim =1
#pragma HLS array_partition variable=tempW6 complete dim =2
	haha tempconv6out[OUT_CH_NUM_Conv6 / 128][OUT_HEIGHT_Conv6][OUT_WIDTH_Conv6] = { 0 };
...
#pragma HLS PIPELINE
						for (int cho = 0; cho < OUT_CH_NUM_Conv6 / times; cho++)//512/16
						{
							...
							{
								tempconv6out[cho][r][c] += tempburelu5out[chi][r + kr][c + kc] * tempW6[cho][chi][kr][kc];
							}
						}
					}
				}
